...
public class PayrollSystem {
    ...
    public void processPayroll(Employee employee, double hoursWorked) {
        ...
        // ✅ Compute total compensation (Gross Salary + Allowances) before deductions
        double totalCompensation = grossSalary + employee.getRiceSubsidy() + employee.getPhoneAllowance() + employee.getClothingAllowance();

        // ✅ Compute deductions using Total Compensation
        double sss = SSSCalculator.getSSSContribution(totalCompensation);
        double philHealth = PhilHealthCalculator.getPhilHealthContribution(totalCompensation);
        double pagIbig = Math.min(totalCompensation * 0.02, 100);
        double taxableIncome = totalCompensation - (sss + philHealth + pagIbig);
